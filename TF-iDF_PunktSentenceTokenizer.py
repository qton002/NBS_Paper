...
filename = "test2.csv"  #spelling checker in excel
...
custom_sent_tokenizer = PunktSentenceTokenizer(no_punct_N_num)
punkt_tokenized = custom_sent_tokenizer.tokenize(no_punct_N_num)
#print(punkt_tokenized)
punkt_tokenized_sents = ' '.join([str(item) for item in punkt_tokenized])
#print(punkt_tokenized_sents)
words = nltk.word_tokenize(punkt_tokenized_sents)
...
vectors_df.columns = ['Words','TF_iDF_PunktSents']
vectors_df.sort_values('TF_iDF_PunktSents', ascending=False,inplace=True)
